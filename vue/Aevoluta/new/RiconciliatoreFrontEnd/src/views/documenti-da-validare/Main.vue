<template>
  ...
  <div class="grid grid-cols-12 gap-6 mt-5">
    ...
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible filter-block-list">
      ...
      <!--end: filter side-->
      <!--Content side-->
      <div :class="active ? 'active' : 'inactive overflow-x-auto'">
        <table class="table table-report -mt-2 doc-val-block">
          <tbody>
            <tr v-for="item in items_t" :key="item">
              ...
              <td class="block text-center lg:flex lg:justify-end">
                <button id="searchDiv" class="btn mr-2 mb-2 w-full" @click="openDoc(item.idfile)">
                  <SearchIcon class="w-4 h-4" /><span v-if="!active">{{
                    $t("documenti_da_validare.Visualizza")
                  }}</span>
                </button>
                <button id="searchDiv" @click="loadDeleteModule(item.idfile)" class="btn mr-2 mb-2 w-full">
                  <Trash2Icon class="w-4 h-4" /><span v-if="!active">{{
                    $t("documenti_da_validare.Scarta")
                  }}</span>
                </button>
                <button id="searchDiv" @click="validaDeleteModule(item.idfile)" class="btn mr-2 mb-2 btn-primary w-full">
...
/* .doc-val-block tr td:last-child {
  display: flex;
    justify-content: space-between;;
}
