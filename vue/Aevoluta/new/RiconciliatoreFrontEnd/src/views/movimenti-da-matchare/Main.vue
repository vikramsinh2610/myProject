<template>
  ...
  <div class="intro-y flex items-center mt-8">
    <!--Start : Top Buttons-->
    <div
      class="w-full intro-y col-span-12 flex flex-wrap sm:mt-0 sm:ml-auto md:ml-0 items-center mt-3"
    >
      <button
        class="btn filter-btn bg-white intro-y flex items-center"
        v-on:click="filterdiv = !filterdiv"
      >
        <SlidersIcon class="w-4 h-4 mr-2" />{{ filterName }}
      </button>

      <h1 class="py-3 px-4 mt-3 xl:mt-0 align-top ml-auto text-base">
        {{ $t("movimento_da_associare.saldo") }}:
        <strong>{{
          saldo != "-" && val != "" ? currencyFormatter(saldo, val) : "-"
        }}</strong>
      </h1>
      <button
        :class="
          validaMatch == 1 && validaPartite == 1
            ? 'btn py-3 text-white px-4 w-full xl:w-44 xm:mt-0 align-top bg-primary'
            : 'btn py-3 px-4 w-full xl:w-44 xm:mt-0 align-top text-black bg-slate-300'
        "
        :disabled="validaMatch == 0 || validaPartite == 0"
        @click="setMatchValidaModal()"
      >
  ...
  <div class="intro-y grid grid-cols-12 gap-9 mt-5">
    ...
    <div
      class="col-span-12 xl:col-span-2 left-part bg-white rounded-lg h-full"
      v-if="!filterdiv"
    >
      <div class="p-5 border-t border-slate-200/60 dark:border-darkmode-400">
        ...
        <div>
          <h2 class="text-md font-medium truncate mr-5 mb-5">
            {{ $t("movimento_da_associare.movimenti_heading") }}
          </h2>
          <!--Start : DropDown fil-->
          ...
          <PreviewComponent v-slot="{ toggle }">
            <div class="mb-3">
              <Preview>
                ...
                >
                  <DropdownToggle
                    class="btn bg-white w-full flex justify-between"
                  >
                    {{ documentoDaCercare["descrizione"] }}
                    {{ documentoDaCercare["iddb"] }}
                    <ChevronDownIcon class="w-4 h-4 ml-2" />
                  </DropdownToggle>
                  <DropdownMenu>
                    <DropdownContent>
                      <DropdownItem
                        v-for="tipi in tipidocumento"
                        :key="tipi.iddb"
                        @click="
                          onChangeTipiDocumento(tipi.iddb, tipi.descrizione)
                        "
                        >{{ tipi.descrizione }} - {{ tipi.iddb }}
                      </DropdownItem>
                    </DropdownContent>
                  </DropdownMenu>
                </Dropdown>
              ...
              <Preview>
                ...
                >
                  ...
                  <DropdownMenu>
                    <DropdownContent>
                      <DropdownItem
                        v-for="anno in annoOption"
                        :key="anno"
                        @click="onChangeAnno(anno)"
                        >{{ anno }}
                      </DropdownItem>
                    </DropdownContent>
                  </DropdownMenu>
          ...
          <div class="mb-3">
            <!-- <Preview> -->
            <!-- <div class="text-center"> -->
            <Dropdown class="" placement="bottom-start" refKey="dropDownRef">
              ...
              <DropdownMenu>
                <DropdownContent>
                  <DropdownItem
                    v-for="mese in meseOption"
                    :key="mese.id"
                    @click="onChangeMese(mese.id, mese.desc)"
                  >
                    {{ mese.desc }}
                  </DropdownItem>
                </DropdownContent>
          ...
          <PlusCircleIcon
            v-if="showMoviPlus"
            class="w-7 h-7 text-blue-700 mx-auto"
            @click="
              showMovimentiFilters = true;
              showMoviPlus = false;
            "
          />
          ...
          <div v-if="showMovimentiFilters">
            ...
            <div class="border border-slate-200/60 rounded mb-3">
              <div class="w-full relative">
                <input
                  type="text"
                  class="form-control w-full box pr-10 text-slate-700"
                  :placeholder="$t('match_validata_page.Ragione_Sociale')"
                  v-model="ragione"
                />
                <SearchIcon
                  class="w-4 h-4 absolute my-auto inset-y-0 right-3"
                />
              </div>
            </div>
            <!--End : Text filter-->
            ...
            <div class="border border-slate-200/60 rounded mb-3 date-range">
              <div class="relative input-text-color w-full">
                <vue-tailwind-datepicker
                  ...
                  v-model="filterDataOpera"
                  :key="filterDataOpera"
                  :placeholder="$t('movimento_da_associare.data_operazione')"
                  input-classes="form-control w-56 box pr-10"
                  use-range
                  :formatter="datepickerFormatter"
                  :start-from="datepickerStartFrom"
                  :disable-date="dDate"
                />
              </div>
            </div>
                  ...
                  >
                    ...
                    <DropdownMenu>
                      <DropdownContent>
                        <DropdownItem
                          @click="
                            valuta = $t('match_validata_page.Tutte');
                            dropDownRef.hide();
                          "
                        >
                        ...
                        <DropdownItem
                          @click="
                            valuta = $t('match_validata_page.Euro');
                            dropDownRef.hide();
                          "
                        >
                        ...
                        <DropdownItem
                          @click="
                            valuta = $t('match_validata_page.Sterlina_Inglese');
                            dropDownRef.hide();
                          "
                        >
            ...
            <div class="border border-slate-200/60 rounded mb-3">
              <div class="w-full relative">
                <input
                  type="text"
                  class="form-control w-full box pr-10 text-slate-700"
                  :placeholder="$t('movimento_da_associare.conto_bancario')"
                  v-model="conto"
                />
                <SearchIcon
                  class="w-4 h-4 absolute my-auto inset-y-0 right-3"
        ...
        <div>
          <h2 class="text-md font-medium truncate mr-5">
            {{ $t("movimento_da_associare.partite_heading") }}
          </h2>
          <!--Start : Text filter-->
          <div class="border border-slate-200/60 rounded mb-3 mt-7">
            <div class="w-full relative">
              <input
                type="text"
                class="form-control w-full box pr-10 text-slate-700"
                :placeholder="$t('Ragioni_Sociali.Ragioni_Sociali')"
                v-model="ragioneP"
              />
          ...
          <div class="border border-slate-200/60 rounded mb-3">
            <div class="w-full relative">
              <input
                type="text"
                class="form-control w-full box pr-10 text-slate-700"
                :placeholder="$t('movimento_da_associare.sap_id')"
                v-model="sapid"
              />
              <SearchIcon class="w-4 h-4 absolute my-auto inset-y-0 right-3" />
            </div>
          ...
          <div class="border border-slate-200/60 rounded mb-3 date-range">
            <div class="relative input-text-color w-full">
              <vue-tailwind-datepicker
                :clearable="true"
                :options="datepickerOptions"
                v-model="filterDataCreazione"
                :key="filterDataCreazione"
                :placeholder="$t('movimento_da_associare.data_creazione')"
                input-classes="form-control w-56 box pr-10 text-slate-700"
                use-range
                :formatter="datepickerFormatter"
                :start-from="datepickerStartFrom"
                :disable-date="dDate"
          ...
          <div class="border border-slate-200/60 rounded mb-3 date-range">
            <div class="relative input-text-color w-full">
              <vue-tailwind-datepicker
                :clearable="true"
                :options="datepickerOptions"
                v-model="filterDataScadenza"
                :key="filterDataScadenza"
                :placeholder="$t('movimento_da_associare.data_scadenza')"
                input-classes="form-control w-56 box pr-10 text-slate-700"
                use-range
                :formatter="datepickerFormatter"
                :start-from="datepickerStartFrom"
          ...
          <PlusCircleIcon
            v-if="showPartitePlus"
            class="w-7 h-7 text-blue-700 mx-auto"
            @click="
              showPartiteFilters = true;
              showPartitePlus = false;
            "
          />
          <div v-if="showPartiteFilters">
                  ...
                  >
                    ...
                    <DropdownMenu>
                      <DropdownContent>
                        <DropdownItem
                          @click="
                            valutaP = $t('match_validata_page.Tutte');
                            dropDownRef.hide();
                          "
                        >
                          {{ $t("match_validata_page.Tutte") }}
                        ...
                        <DropdownItem
                          @click="
                            valutaP = $t('match_validata_page.Euro');
                            dropDownRef.hide();
                          "
                        >
                          {{ $t("match_validata_page.Euro") }}
                        ...
                        <DropdownItem
                          @click="
                            valutaP = $t(
                              'match_validata_page.Sterlina_Inglese'
                            );
                            dropDownRef.hide();
                          "
                        >
            ...
            <MinusCircleIcon
              class="w-7 h-7 text-blue-700 mx-auto"
              @click="
                showPartiteFilters = false;
                showPartitePlus = true;
              "
            />
          </div>
        </div>
        <button
          class="btn btn-sm btn-primary w-18 mt-3 float-right mb-2"
          @click="applyFilters()"
        >
          {{ $t("commons.applica") }}
        </button>
    ...
    <div
      :class="
        filterdiv == true
          ? 'col-span-12 xl:col-span-5 h-full'
    ...
    >
      <div class="intro-y box h-full">
        <div class="border-b p-4">
          ...
          >
            <div class="flex flex-col">
              ...
              <h2 class="intro-y flex items-center text-slate-400 text-xs">
                {{ m_total }}
                {{ $t("movimento_da_associare.movimenti_totali") }}
              </h2>
            </div>
            <div
            ...
            >
              <label class="form-check-label ml-0" for="show-example-1">{{
                $t("movimento_da_associare.toggle_heading")
              }}</label>
              <input
                @click="toggle"
                class="form-check-input mr-0 ml-3"
                type="checkbox"
                v-model="toggleMovimento"
        ...
        <div class="p-0 mt-3 h-full">
          <div
            class="intro-y col-span-12 overflow-auto 2xl:overflow-visible h-full"
          >
            ...
            <AccordionGroup selectedIndex="null">
              <AccordionItem
                v-for="(item, index) in items_t"
                :key="item"
                class="intro-x p-4"
              >
                <Accordion class="pb-1 relative flex items-center accordionBtn">
                  <input
                    class="form-check-input mr-4 border-2"
                    type="checkbox"
                    :id="'Movimenti' + index"
                    @click="
                      checkMovimenti(
                  ...
                  <div class="flex flex-col w-full">
                    ...
                    <div class="flex flex-row w-full">
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs whitespace-nowrap mt-0.5"
                      >
                        {{ $t("movimento_da_associare.importo") }} :
                      ...
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs whitespace-nowrap mt-0.5"
                      >
                        {{ $t("movimento_da_associare.data_doc") }} :
                        <span class="text-slate-800 font-medium"
                          >{{
                      ...
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs whitespace-nowrap mt-0.5"
                      >
                        {{ $t("movimento_da_associare.valuta") }} :
                        <span class="text-slate-800 font-medium"
                          >{{ item.valuta }}
                        </span>
                      </div>
                ...
                <AccordionPanel class="text-slate-600 dark:text-slate-700">
                  <div class="ml-[32px]">
                    <div class="flex gap-y-1 flex-row flex-wrap">
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs mt-0.5"
                      >
                        {{ $t("movimento_da_associare.id_mov") }} :
                        <span class="text-slate-800 font-medium"
                          >{{ item.iddbmovimento }}
                        </span>
                      </div>
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs mt-0.5"
                      >
                        {{ $t("movimento_da_associare.data_op") }} :
                        <span class="text-slate-800 font-medium"
                          >{{
                            moment(item.dataregistrazione).format("DD/MM/YYYY")
                          }}
                      ...
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs mt-0.5"
                      >
                        {{ $t("movimento_da_associare.conto") }} :
                        <span class="text-slate-800 font-medium"
                          >{{ item.conto }}
                        </span>
                      </div>
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs mt-0.5"
                      >
                        {{ $t("movimento_da_associare.tipo_doc") }} :
                        <span class="text-slate-800 font-medium"
                          >{{ item.tipofile }}
                        </span>
                      </div>
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs mt-0.5"
                      >
                        {{ $t("movimento_da_associare.sap_id") }} :
                        <span class="text-slate-800 font-medium"
                          >{{ item.sapid }}
                        </span>
                      </div>
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs mt-0.5"
                      >
                        {{ $t("movimento_da_associare.micro") }} :
                        <span class="text-slate-800 font-medium"
                          >{{
                            item.is_sottomovimento_con_supermovimento_aggregato
                          }}
                        </span>
                      </div>
                      <div
                        class="text-slate-500 w-full font-normal text-xs mt-0.5"
                      >
                        {{ $t("movimento_da_associare.note") }} :
                        <span class="text-slate-800 font-medium"
                          >{{ item.notemovimento }}
                        </span>
                      </div>
                    ...
                    <div class="flex gap-4 justify-end mt-4">
                      <button
                        id="searchDiv"
                        class="btn mb-2 flex items-center gap-1 px-5 py-1"
                        @click="
                          openDisattivaModal(
                            true,
                            item.is_movimento_disattivato,
                            item.artificial_id_movimento
                          )
                        "
                      >
                        <SlashIcon class="w-4 h-4" /><span v-if="filterdiv">{{
                          $t("movimento_da_associare.disattiva")
                        }}</span>
                      </button>
                      <button
                        id="searchDiv"
                      ...
                      >
                        <EditIcon class="w-4 h-4" /><span v-if="filterdiv">{{
                      ...
                      >
                        <FileTextIcon class="w-4 h-4" /><span
                          v-if="filterdiv"
                          >{{
                            $t("movimento_da_associare.visualizza_doc")
                          }}</span
                        >
                      </button>
                    </div>
    ...
    <div
      :class="
        filterdiv == true
          ? 'col-span-12 lg:col-span-2 relative top-1/3 h-40'
          : 'col-span-12 lg:col-span-2 relative top-1/3 h-40'
      "
    >
      <div class="">
        <div
          class="rounded-full flex justify-center items-center w-24 h-4"
        ></div>
        <div
    ...
    <div
      :class="
        filterdiv == true
          ? 'col-span-12 xl:col-span-5 flex h-full'
          : 'col-span-12 lg:col-span-4 h-full'
      "
    >
        ...
        <div class="border-b p-4">
          ...
          >
            <div class="flex flex-col">
              ...
              <h2 class="intro-y flex items-center text-slate-400 text-xs">
                {{ p_total }} {{ $t("movimento_da_associare.partite_totali") }}
              </h2>
            </div>
              ...
              >
                <Accordion class="pb-1 relative flex items-center accordionBtn">
                  <input
                    ...
                    type="checkbox"
                    :id="'Partite' + index"
                    @click="checkPartite(itemP.iddb, index, itemP.valutalordo)"
                  />
                  <div class="flex flex-col w-full">
                    ...
                    <div class="flex flex-row w-full">
                      ...
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs whitespace-nowrap mt-0.5"
                      >
                        {{ $t("movimento_da_associare.id_doc") }} :
                        <span class="text-slate-800 font-medium"
                          >{{ itemP.iddocumento }}
                        </span>
                      </div>
                  ...
                  <div class="ml-[32px]">
                    <div class="flex gap-y-1 flex-row flex-wrap">
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs mt-0.5"
                      >
                        {{ $t("movimento_da_associare.data_scad") }} :
                        <span class="text-slate-800 font-medium"
                          >{{ moment(itemP.datascadenza).format("DD/MM/YYYY") }}
                        </span>
                      </div>
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs mt-0.5"
                      >
                        ...
                        <span class="text-slate-800 font-medium"
                          >{{
                            moment(itemP.datadocumento).format("DD/MM/YYYY")
                          }}
                        </span>
                      </div>
                      <div
                        class="text-slate-500 w-[33.33%] font-normal text-xs mt-0.5"
                      >
                        {{ $t("movimento_da_associare.testo_par") }} :
                        <span class="text-slate-800 font-medium"
                          >{{ itemP.testo_partita }}
                        </span>
                      </div>
                    ...
                    <div class="flex gap-4 justify-end mt-4">
                      <button
                        id="searchDiv"
                        class="btn mr-2 mb-2 flex items-center gap-1 px-5 py-1"
    ...
    <ModalBody class="p-0">
      <div class="p-5 text-center">
        ...
        <div class="text-slate-500 mt-2">
          {{ $t("movimento_da_associare.disattiva_question") }}<br />{{
            $t("movimento_da_associare.statement")
          }}
        </div>
      </div>
      <div class="px-5 pb-8 text-center">
        ...
        >
          {{ $t("match_validati_details.Annulla") }}
        </button>
        <button type="button" class="btn btn-danger w-24" @click="deactivate()">
          {{ $t("movimento_da_associare.disattiva") }}
        </button>
      </div>
    </ModalBody>
  ...
  <Modal :show="modificaModal" @hidden="modificaModal = false">
    <ModalBody class="p-0">
      ...
      <div class="border-y-2">
        <div class="p-5">
          <div class="text-slate-500 mt-2">
            {{ $t("movimento_da_associare.modifica_statement") }}
          </div>
          <div>
            <!-- <PreviewComponent v-slot="{ toggle }"> -->
            ...
            <div class="mb-3">
              <!-- <Preview> -->
              <!-- <div class="text-center"> -->
              <!-- <Dropdown class="" placement="bottom-start">
                <DropdownToggle
                  class="btn bg-white w-full flex justify-between"
                >
                  {{ stakeData.name }}
                ...
                <DropdownMenu>
                  <DropdownContent>
                    <DropdownItem
                      v-for="tipi in stakeHolders"
                      :key="tipi.id"
                      @click="onChangeStakeData(tipi)"
                      >{{ tipi.ragionesociale }}
                    </DropdownItem>
                  </DropdownContent>
                </DropdownMenu>
              ...
              <select
                class="dropdown-toggle rounded bg-white w-full flex justify-between focus:border-0 mt-2 text-left text-sm"
                @change="onChangeStakeData($event)"
              >
                <option value="0">{{ stakeData.name }}</option>
                <option
                  v-for="tipi in stakeHolders"
                  :key="tipi.id"
                  :value="tipi?.id"
      ...
      <div class="px-5 pt-5 pb-8 text-end flex justify-between">
        <button
          type="button"
          @click="modificaModal = false"
  ...
  <Modal
    class="validaMatch"
    :show="manualMatchModal"
    @hidden="manualMatchModal = false"
  >
    <ModalBody class="p-0">
      <div class="flex justify-between items-center border-b p-4">
        <h4 class="text-xl">{{ $t("movimento_da_associare.valida_match") }}</h4>
        <div class="text-right">
          {{ $t("automatici_da_validar.saldo_iniziale") }}:
          <span>
            {{
              saldo != "-" && val != "" ? currencyFormatter(saldo, val) : "-"
            }}</span
      ...
      <div
        v-if="addDifferenze.length > 0"
        class="p-4"
        v-for="(add, index) in addDifferenze"
      >
        <div class="sm:flex items-center">
            ...
            <Preview>
              ...
              <Dropdown class="" placement="bottom-start" v-slot="{ dismiss }">
                ...
                <DropdownMenu>
                  <DropdownContent>
                    <DropdownItem
                      v-for="tipi in differenzePartiteCodici"
                      :key="tipi.codicesapconto"
                      @click="
                        onChangeCodice(tipi.codicesapconto, tipi.nomeconto, add)
                      "
                      >{{ tipi.nomeconto }}
                    </DropdownItem>
                  </DropdownContent>
          ...
          <!-- END: Dropdown with close button -->
          <div class="w-12 flex-none xl:flex-initial sm:w-1/4 text-center">
            {{ $t("automatici_da_validar.value") }}
          </div>
          <input
            id="tabulator-html-filter-value"
            type="number"
            class="form-control sm:w-1/3 mt-2 sm:mt-0"
            placeholder=""
            v-model="add.differenzeValue"
            :disabled="!(add.nomeconto['codicesapconto'] != '')"
          />
          <MinusCircleIcon
            class="w-7 h-8 text-primary ml-auto"
            @click="RemoveDifferenzeItem(add)"
          />
        </div>
      </div>
      <div class="p-4">
  ...
  <!-- BEGIN: Delete Notification Content -->
  <Notification
    refKey="basicDeleteNotification"
    :options="{
      duration: 3000,
    }"
    class="flex flex-col sm:flex-row bg-green-500 border-0"
  >
    <div class="font-normal text-lg">
      {{ $t("automatici_da_validar.notify_disassocia") }}
    </div>
    <!-- <a class="font-medium text-primary dark:text-slate-400 mt-1 sm:mt-0 sm:ml-40" href="">Review Changes</a> -->
  </Notification>
  <!-- END: Delete Notification Content Content -->

  <!-- BEGIN: Valid Notification Content -->
  <Notification
    refKey="basicValidNotification"
    :options="{
      duration: 3000,
    }"
    class="flex flex-col sm:flex-row bg-green-500 border-0"
  >
    <div class="font-normal text-lg">
      {{ $t("automatici_da_validar.notify_match") }}
    </div>
    <!-- <a class="font-medium text-primary dark:text-slate-400 mt-1 sm:mt-0 sm:ml-40" href="">Review Changes</a> -->
  </Notification>
  <!-- END: Valid Notification Content Content -->
  <!-- BEGIN: Error Notification Content -->
  <Notification
    refKey="basicValidErrorNotification"
    :options="{
      duration: 3000,
    }"
    class="flex flex-col sm:flex-row bg-red-500 border-0"
  >
    <div class="font-normal text-lg">{{ $t("commons.error") }}</div>
  </Notification>
  <!-- END: Notification Content Content -->
</template>
<script setup>
import { ref, watch, onMounted, onBeforeMount, computed, provide } from "vue";
...
const basicValidNotification = ref();
const basicValidErrorNotification = ref();
provide("bind[basicDeleteNotification]", (el) => {
  // Binding
  basicDeleteNotification.value = el;
});
provide("bind[basicValidNotification]", (el) => {
  // Binding
  basicValidNotification.value = el;
});
provide("bind[basicValidErrorNotification]", (el) => {
  // Binding
  basicValidErrorNotification.value = el;
});
...
saldoFinal.value = computed(() => {
  console.log("SaldoFinal  :   ", saldo.value);
  let final = globalRounderToTwoDecimals(
    Number(saldo.value) - Number(differenzaTotale())
  );
  if (final == 0) {
...
watch(toggleMovimento, (newValue) => {
  if (newValue == true) {
    //show the selected value for movimenti
    if (selectedMovimento.value != undefined) {
      let items = items_t.value.filter(
        (movi) => movi.iddbmovimento === selectedMovimento.value
      );
      items_t.value = items;
    }
  } else {
...
function globalRounderToTwoDecimals(num) {
...
async function getMovimentiData() {
  ...
  //tipo
  if (
    documentoDaCercare.value.descrizione == "Tipo Documento" ||
    documentoDaCercare.value.iddb == "0"
  ) {
  } else {
  ...
  if (filterDataOpera._rawValue.length > 0) {
    ...
    params.append(
      "dataoperazioneda",
      moment(dateSplit[0]).format("YYYY-MM-DDTHH:MM:SS")
    );
    params.append(
      "dataoperazionea",
      moment(dateSplit[1]).format("YYYY-MM-DDTHH:MM:SS")
    );
  }
  //valuta
  ...
  params.append(
    "includi_movimenti_coinvolti_in_match_confermati_manualmente",
    false
  );
  ...
  m_total.value = matchStore.movimenti.total;
  val.value = items_t.value[0].valuta;
...
async function getPartiteData() {
  ...
  if (filterDataCreazione._rawValue.length > 0) {
    let dateSplit = filterDataCreazione._rawValue
      .toString()
      .trim()
      .split(" ~ ");
    params.append(
  ...
  //dataScadenza
  if (filterDataScadenza._rawValue.length > 0) {
    let dateSplit = filterDataScadenza._rawValue.toString().trim().split(" ~ ");
    params.append(
      "datascadenzada",
      moment(dateSplit[0]).format("YYYY-MM-DDTHH:MM:SS")
    );
  ...
  p_total.value = matchStore.partite.total;
  // debugger
}
...
function reset() {
  ...
  conto.value = "";
  ragioneP.value = "";
  filterDataScadenza.value = "";
  filterDataCreazione.value = "";
  sapid.value = "";
  getPartiteData();
  getMovimentiData();
}
function onChangeStakeData(event) {
  if (event.target.value != 0) {
    stakeData.value.id = event.target.value;
  }
  // stakeData.value.id = event?.id;
  // stakeData.value.name = event?.ragionesociale;
}
...
async function cambiaRS() {
  ...
  await matchStore.UpdateMovimento({
    idMov: madificaSapID.value.artificial_id_movimento,
    op: "cambiaRS",
    idStakeholder: stakeData.value.id,
  });
...
function AddDifferenzeItem() {
  addDifferenze.value.push({
    nomeconto: { nomecontoD: "Codice", codicesapconto: "" },
    differenzeValue: "",
  });
}
function RemoveDifferenzeItem(item) {
  ...
  console.log(
    "Removed!! Remaining array length:    ",
    addDifferenze.value.length
  );
}
function differenzaTotale() {
  ...
  for (const item of addDifferenze.value) {
    ...
    if (item.nomeconto.codicesapconto && item.nomeconto.codicesapconto != "") {
      console.log(
        "Passato: " +
          item.nomeconto.codicesapconto +
          "  differenzeValue === " +
          item.differenzeValue
      );
...
function differenzeSintetiche() {
  ...
  for (const item of addDifferenze.value) {
    ...
    let cod = differenzePartiteCodici.value.find(
      (diff) => diff.codicesapconto === cod_sap
    ).codiceinternoconto;
...
async function matchValida() {
  ...
  let response = 0;
  await matchStore
    .CreaESalvaMatch(creaEChiudi)
    .then((res) => {
      //if there is an error, showing the error notification (response = 1)
      if (res == false) {
        response = 1;
      }
    })
    ...
    .finally(() => {
      if (response != 0) {
        basicValidErrorNotification.value.showToast();
      } else {
        basicValidNotification.value.showToast();
      }
    });
...
function currencyFormatter(data, cFormate) {
  console.log("currency Formatter : ", data);
