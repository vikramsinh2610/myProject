<template>
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div class="col-span-12 sm:col-span-6 xl:col-span-3 intro-y">
            <div class="report-box zoom-in" style="cursor:default">
                <div class="box box-card p-5 h-32">
                    <div class="text-3xl font-medium leading-8" :class="detail?.partite_di_pareggio?.length ? 'mt-auto':'mt-6'">
                        {{ currencyFormatter(detail?.differenzamonetizzatainvalutaconimportoscontato, 'EUR') }}</div>
                    <div class="text-base text-slate-500 mt-1">{{ $t("match_validata_page.Saldo_sconto") }}</div>
                    <div class="text-3xl font-medium leading-8 mt-auto" v-if="detail?.partite_di_pareggio?.length">
                        {{ currencyFormatter(detail?.saldo, 'EUR') }}</div>
                    <div class="text-base text-slate-500 mt-1" v-if="detail?.partite_di_pareggio?.length">{{ $t("match_validata_page.Saldo_piu_Movimenti") }}</div>
                </div>
            </div>
        </div>
        <div class="col-span-12 sm:col-span-6 xl:col-span-3 intro-y">
                        ...
                        <table class="table-list-block w-full">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="py-1 w-40">
                                        {{ $t("movimento_da_associare.testo_par") }}: <span class="font-medium">{{
    ...
    <Notification refKey="basicDeleteNotification" :options="{
        ...
        <div class="font-normal text-lg">{{ $t("automatici_da_validar.notify_disassocia") }}</div>
        ...
        <ModalBody class="p-0">
            ...
            <div v-if="addDifferenze.length > 0" class="p-4" v-for="(add, index) in addDifferenze">
                ...
                <div class="sm:flex items-center">
                    ...
                    <div class="w-12 flex-none xl:flex-initial sm:w-1/4 text-center">{{ $t("automatici_da_validar.value") }}
                    </div>
...
onBeforeMount(async () => {
    ...
    let response = 0;
    await matchStore.LoadMatches(newObj).then(
      (res) => {
    ...
    .finally(() => {
        if(response == 0){
            ...
            let theMatch = items_t.value.find(item => {
                console.log(item.idmatch)
                return item.idmatch == pathname[2]
            })

            detail.value = theMatch
            saldo.value = detail.value.saldo
            if (detail.value.score > 0 && detail.value.score <= 40) {
                confidence.value = 'bassa'
            } else if (detail.value.score > 40 && detail.value.score <= 60) {
                confidence.value = 'intermedia'
            } else if (detail.value.score > 60 && detail.value.score <= 100) {
...
function setMatchValidaModal(){
    ...
    if (detail.value.saldo != "" && detail.value.saldo != 0) {
        prepareDifferenze();
...
function backValidare() {
    if (confidence.value == 'elevata') {
        router.push({ name: "Match con Confidenza Elevata" });
    } else if (confidence.value == 'intermedia') {
        router.push({ name: 'Match con Confidenza Intermedia' });
    } else if (confidence.value == 'bassa') {
        router.push({ name: 'Match con Confidenza Bassa' });
    }
}
function currencyFormatter(data, cFormate) {
    let newCurrency = new Intl.NumberFormat(undefined, { style: 'currency', currency: cFormate }).format(data);
    return newCurrency
}
...
async function deleteMatch() {
    ...
    try {
        ...
        console.log("Cancella Match for id: " + theId[2])
        let theMatch = matchStore.getMatches.items.find(item => { return item.idmatch == theId[2] })
...
.toastify-content.bg-red-500 {
    background-color: #cc0000 !important;
}

.toastify-content.bg-green-500 {
    background-color: #91c71a !important;
}
</style>