<template>
  <div class="intro-y block md:flex md:justify-between items-center py-4">
    <div class="flex justify-between items-center">
      ...
      <h1 class="text-lg font-medium truncate ml-2">{{ $t("automatici_da_validar.match_con_confidenza") }} {{
        $t("home-page." + confidence) }}</h1>
    </div>

    <button :disabled="mutilpleValidMatchBtn == false"
      :class="(mutilpleValidMatchBtn) ? 'btn btn-primary w-full md:w-56 mr-2 mb-2 h-10 ml-auto mt-4 md:mt-0' : 'btn btn-secondary w-full md:w-56 mr-2 mb-2 h-10 ml-auto mt-4 md:mt-0'"
  ...
  <div class="grid grid-cols-12 gap-6">
    ...
    <div class="intro-y col-span-12  block justify-between md:flex md:justify-between">
      <div class="col-span-12 xl:col-span-4 left-part bg-white mr-5 rounded-lg md:w-1/4 w-full h-80 mb-5"
        v-if="!filterdiv">
        <div class="p-5 border-t border-slate-200/60 dark:border-darkmode-400">
          <div class="flex items-center py-4">
            ...
            <button class="text-primary w-45 ml-auto" @click="reset()"> {{ $t("commons.reset") }} </button>
          </div>
          <div>
            ...
            <div class="border border-slate-200/60 rounded mb-2">
              <div class="relative w-full">
                <input type="text" class="form-control box pr-10 text-slate-900"
      ...
      <div class="w-full overflow-x-auto" :class="active ? '' : 'inactive'">
        <table class="table table-report -mt-2">
          <tbody>
            <tr v-for="(item, index) in items_t" :key="item" class="intro-x">
              ...
              <td class="w-24">
                <div class="flex pl-4">
                  <div class="w-16 h-10 image-fit flex justify-between items-center">
                    <input v-if="item.saldo == 0" class="form-check-input border-2" type="checkbox"
                      :id="'Partite' + index" @click="checkMatch(item.idmatch)" />
                    <span></span>
                    <div class="w-7 h-7 image-fit nonclickCursor">
              ...
              <template v-for="movi in item.movimenti" :key="movi">
                ...
                <td class="w-40">
                  ...
                  <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">
                    {{ $t("match_validata_page.ID_Movimento") }}: <b>{{ movi.iddbmovimento }}</b>
                  </div>
                </td>
              </template>
              <!--end Movimenti-->
              ...
              <td class="">
                <div class="partion-block1 mx-auto">

                  <div class="flex items-center gap-7 justify-between" v-for="par in item.partite" :key="par">
                    <div class="option-1 mt-4">
                      <div class="font-medium whitespace-nowrap">
                        {{ par.ragionesociale }}
                      </div>
                      <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">
              ...
              <td class="w-40 border border-t-0 border-r-0">
                <div class="font-medium whitespace-nowrap" v-if="item.saldo == 0">
                ...
                <div class="font-medium whitespace-nowrap" v-else>
                  {{ $t("match_validata_page.Saldo_Cond_Pag") }}:
                  <b>{{
                    currencyFormatter(
  ...
  <Modal :show="autoMatchModal" @hidden="autoMatchModal = false">
    <ModalBody class="p-0">
      ...
      <div class="px-5 pb-8 text-center">
        ...
        <button type="button" class="btn btn-primary w-24" @click="zeroSaldoValida()">{{
  ...
  <Notification refKey="basicValidNotification" :options="{
    duration: 3000,
  }" class="flex flex-col sm:flex-row bg-green-500 border-0 mt-10">
    <div class="font-normal text-lg">{{ $t("automatici_da_validar.notify_match") }}</div>
  ...
  <Notification refKey="basicValidErrorNotification" :options="{
    duration: 3000,
  }" class="flex flex-col sm:flex-row bg-red-500 border-0 mt-10">
...
import { ref, watch, onMounted, computed, onBeforeMount, provide } from "vue";
...
saldoFinal.value = computed(() => {
  let final = globalRounderToTwoDecimals(Number(saldo.value) - Number(differenzaTotale()))
  if (final == 0) {
...
function loadDetailPage(matchId) {
  ...
  router.push({ name: "Dettaglio Match", params: { id: matchId } });

}
async function getMatchData() {

  let pathname = window.location.pathname.split('/');
  let getScore = pathname[2]
  // debugger
  ...
  let minscoring = 0;
  let maxscoring = 0;
  confidence.value = getScore
  if (getScore === "elevata") {
    minscoring = 60;
    maxscoring = 100;
    pageheading.value = "Elevata"

  }
  if (getScore === "intermedia") {
    minscoring = 40;
    maxscoring = 60;
    pageheading.value = "Intermedia"
    //debugger
  }
  if (getScore === "bassa") {
    minscoring = 0;
    maxscoring = 40;
    pageheading.value = "Bassa"
  }
  ...
  await matchStore.LoadMatches(newObj)
    ...
    .finally(() => {
      if (response == 0) {
        ...
        totalPages.value = matchStore.matches.total / matchStore.matches.size;
        if (items_t.value.length > 0) {
          isRecord.value = true;
        } else {
          isRecord.value = false;
        }
        startPagenumber.value = matchStore.matches.size * (matchStore.matches.page - 1) + 1;
        if (
          matchStore.matches.page * matchStore.matches.size > matchStore.matches.total
        ) {
          endPagenumber.value = matchStore.matches.total;
        } else {
          endPagenumber.value = startPagenumber.value + matchStore.matches.size - 1;
        }
      } else if (response == 1) {
...
function setMatchValidaModal(item) {
  detail.value = item
  if (item.saldo != "" && item.saldo != 0) {
    prepareDifferenze();
    console.log(addDifferenze.value)
    saldo.value = item.saldo
    manualMatchModal.value = true
    autoMatchModal.value = false
    console.log("Saldo not zero : ", saldo.value)
    //debugger
  } else {
    console.log("Saldo is zero ", saldo.value)
    //debugger
...
function multiMatchValida() {
  ...
  if (selectedPartite.value != undefined) {
    ...
    let actual = 0;
    items.forEach(async function (detail, index) {

      await matchStore.SaveMatch(detail)
        .then(

          (res) => {
            //if there is an error, showing the error notification
            if (res == false) {
              response = 1
            }
          },
...
async function zeroSaldoValida() {
  let response = 0
  await matchStore.SaveMatch(detail.value)
    .then(
      (res) => {
        //if there is an error, showing the error notification (response = 1)
        if (res == false) {
...
function prepareDifferenze() {

  const differenzeInIngresso = detail.value.partite_di_pareggio
  //debugger
  console.log("prepareDifferenze: " + detail.value)
  if (differenzeInIngresso && differenzeInIngresso.length > 0) {
    // addDifferenze.value =ref([]);    
    for (const item of differenzeInIngresso) {
      let codice = differenzePartiteCodici.value.find(diff => diff.codiceinternoconto === item.cod);
      if ((codice.codicesapconto == "5564515" || codice.codicesapconto == "4519500") && (Math.abs(Number(item.importo)) > 1)) {
        addDifferenze.value.push({ nomeconto: { nomecontoD: "", codicesapconto: "" }, differenzeValue: Number(item.importo) })
      } else if (codice.codicesapconto != "6200000" && codice.codicesapconto != "6140000") {
        if (item.id_partita) {
          addDifferenze.value.push({ idPartita: item.id_partita, nomeconto: { nomecontoD: codice.nomeconto, codicesapconto: codice.codicesapconto }, impdifferenzeValueorto: Number(item.importo) })
        } else {
          addDifferenze.value.push({ nomeconto: { nomecontoD: codice.nomeconto, codicesapconto: codice.codicesapconto }, differenzeValue: Number(item.importo) })
        }
      }
    }
    console.log("if : ", addDifferenze.value)
  } else {
    if (saldo.value < 0) {
      if (saldo.value < -1) {
        addDifferenze.value = [{ nomeconto: { nomecontoD: "", codicesapconto: "" }, differenzeValue: 0 }];
      } else {
        addDifferenze.value = [{ nomeconto: { nomecontoD: "Arrotondamenti/Abbuoni passivi", codicesapconto: "5564515" }, differenzeValue: 0 }];
      }
    } else {
      if (saldo.value > 1) {
        addDifferenze.value = [{ nomeconto: { nomecontoD: "", codicesapconto: "" }, differenzeValue: 0 }];
      } else {
        addDifferenze.value = [{ nomeconto: { nomecontoD: "Arrotondamenti/Abbuoni attivi", codicesapconto: "4519500" }, differenzeValue: 0 }];
      }
    }
    console.log(addDifferenze.value);
  }
