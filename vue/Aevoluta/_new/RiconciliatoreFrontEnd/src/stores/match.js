...
export const useMatchStore = defineStore("match", {
  ...
  getters: {
    ...
    getMatchesFiltrati(state) {
      console.log("Numero partite da filtrare: " + Object.keys(state.matches).length + " con filtro: " + state.filtroMARagioneSociale + " e " + state.filtroMAValuta)
      ...
      let matchesFiltrati = state.matches.items.filter(match => {
        ...
        if(state.filtroMAValuta && state.filtroMAValuta.length > 2) {
          for(const partita of match.partite) {
            ok = ok && partita.valuta && (partita.valuta
            .toLowerCase()
  ...
  actions: {
    ...
    async CreaESalvaMatch ({commit}, creaEChiudi) {
      try{
        ...
        if(creaEChiudi.differenze) {
          params.partite_pareggio = partitedipareggio;
        }
    ...
    async LoadMatchesByConfidence (params) {
      axios.get('matches-by-confidence/', { params })
        .then((response) => this.matchesByConfidence = response.data)
        .catch((error) => this.matchesByConfidence = {
          "count": 0,
          "relative_low_confidence": 0,
          "relative_medium_confidence": 0,
          "relative_high_confidence": 0,
