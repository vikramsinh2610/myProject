...
const matchLow = computed(() => matchStore.getMatchesByConfidence.low_confidence);
const matchMedium = computed(() => matchStore.getMatchesByConfidence.medium_confidence);
const matchHigh = computed(() => matchStore.getMatchesByConfidence.high_confidence);
...
const caricaMatch = async (tipo) => {
  ...
  router.push({name: "matchassociated1", params: {numMatch: 10}});
}
