...
module.exports = (fastify, opts, next) => {
  const options = {
    ...
    schema: {
      summary: 'Invoicing Close',
      description: 'Close invoicing for the specified productive period',
      tags: ['invoicing'],
      params: {
        type: 'object',
        properties: {
  ...
  fastify.post('/', options, errorHandler(async (request, reply) => {
    ...
    logRepository.insert(
      ...
      new LogEvent({
        ...
        description: `INIZIO chiusura fatturazione ${request.params.invoicingId}`,
      }),
    );
    ...
    worker.send({
      action: 'closeInvoicing',
      stateInvoicing,
      invoicingId: request.params.invoicingId,
      edition: fastify.edition,
    });

    reply.send({ _meta: {}, item: { ...stateInvoicing, status: statuses.CLOSE_PROCESSING } });
  }));
  next();
};
