...
function getCurrentOrdinal(mongodb, month, year) {
  return counters.current(mongodb, `EXPORT-INVOICES-TCA-ID-${month}-${year}`).then((count) => count);
}

function getLastOrdinal(mongodb) {
  return counters.lastInvoiceCounterTCA(mongodb).then((count) => count);
}

function getPreviousOrdinal(mongodb) {
  return counters.previousInvoiceCounterTCA(mongodb).then((count) => count);
}

function setNumber(mongodb, month, year, number) {
  return counters.set(mongodb, `EXPORT-INVOICES-TCA-ID-${month}-${year}`, number).then((count) => count);
}
  ...
  fastify.post(
    ...
    errorHandler(async (request, reply) => {
      ...
      invoiceService
        ...
        .getInvoices(0, 0, { productivePeriodYear, productivePeriodMonth, 'heading.prefix': 'TCW' }) // request.query.count
        .then(async (invoices) => {
          // request.query.count
          const rows = [];
          ...
          await invoices.forEachAsync(async (i) => {
            ...
            if (survey && survey.length > 0) {
              ...
              const onFiscaleTcaResponses = survey[0].questions.find((el) => el._id === 'codice-soggetto-tca');
              if (
              ...
              if (
                onFiscaleTcaResponses &&
                onFiscaleTcaResponses.responses &&
                onFiscaleTcaResponses.responses.length > 0
              ) {
                const onFiscaleTcaResponse = onFiscaleTcaResponses.responses.find((el) => el.selected);
                if (onFiscaleTcaResponse) {
                  // eslint-disable-next-line no-unused-vars
                  onFiscaleTca = onFiscaleTcaResponse.typeValue;
                }
              }
            }
            ...
            await i.accountingNotes.forEachAsync(async (n) => {
              if (n.invoiceAmount !== 0) {
                ...
                rows.push({
                  ...
                  codiceSoggettoIntestatario: onFiscaleTca,
                  valutaDocumento: 'EUR',
                  cambioValutaConto: '1',
                  dataOperazione: moment(i.issueDate).format('YYYY-MM-DD'),
                  businessUnit: '001',
                  codiceUnitOperativa: '0000000025',
