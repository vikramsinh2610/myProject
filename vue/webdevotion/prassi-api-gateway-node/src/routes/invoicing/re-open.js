  ...
  fastify.post('/', options, errorHandler(async (request, reply) => {
    ...
    logRepository.insert(
      ...
      new LogEvent({
        ...
        description: `INIZIO ROLLBACK/OPEN fatturazione ${request.params.invoicingId}`,
      }),
    );
    ...
    worker.send({
      action: 're-openInvoicing',
      stateInvoicing,
      invoicingId: request.params.invoicingId,
      edition: fastify.edition,
    });

    reply.send({ _meta: {}, item: { ...stateInvoicing, status: statuses.PROCESSING } });
  }));
  next();
};
