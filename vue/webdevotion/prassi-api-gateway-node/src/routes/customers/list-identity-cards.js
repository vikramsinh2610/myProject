    ...
    errorHandler(async (request, reply) => {
      ...
      const filter = {
        ...(request.query.searchCustomer
          ? {
              NomeCliente: { $regex: request.query.searchCustomer, $options: 'i' },
            }
          : {}),
        ...customerIDs,
      ...
      const filterIdCard = {
        ...(request.query.expired === 'expired'
          ? {
              DataScadenza: { $lt: today.toDate() },
            }
          : {}),
        ...(request.query.expired === 'expiring' && request.query.complete === 'complete'
          ? {
              $and: [
                {
        ...
        ...(request.query.expired === 'expiring' && request.query.complete !== 'complete'
          ...
          : {}),
        ...(request.query.complete === 'complete' && request.query.expired !== 'expiring'
          ? {
              $and: [
                { DataScadenza: { $ne: null } },
                { TipoDocumento: { $ne: null } },
          ...
          : {}),
        ...(request.query.complete === 'incomplete'
          ? {
              $or: [
                { DataScadenza: null },
                { DataScadenza: null },
      ...
      const customerItems =
        fastify.edition === 'tcw'
          ? await customerService
              .getCustomersIdentityCardsPrivacy(
              ...
              .catch((error) => error)
          : await customerService
              .getCustomersIdentityCardsMandato(
                request.query.type !== 'inherited' ? request.query.skip : 0,
                request.query.type !== 'inherited' ? request.query.count : 0,
                filter,
                filterIdCard,
      ...
      while (customerItems[startFrom] && customerSliceEnriched.length < endCount) {
        ...
        // eslint-disable-next-line no-await-in-loop
        const customer = await customerService.getCustomerById(item._id);
      ...
      return reply.send({
        ...
        items:
          request.query.type !== 'inherited'
            ...
            : customerSliceEnriched.slice(-request.query.count),
      });
    }),
  );
  next();
