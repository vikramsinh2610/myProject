  ...
  fastify.get(
      ...
      errorHandler(async (req, reply) => {
        ...
        try {
          ...
          await page.goto(
              `${req.secure ? 'https://' : 'http://'}${req.headers.host}/v1/survey/autocert-per-giuridica-to-html/${
              ...
              .addDocument(
                  {
                    ...
                    ownerId: 'SYSTEM',
                    displayName: `autocert-per-giuridica-${name}-${surname}-${translateSurveyTypeId}.pdf`,
                    locked: true,
                  },
                  pdf,
              )
