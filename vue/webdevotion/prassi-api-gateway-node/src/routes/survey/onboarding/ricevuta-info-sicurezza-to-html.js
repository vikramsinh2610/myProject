  ...
  fastify.get(
    ...
    errorHandler(async (request, reply) => {
      ...
      try {
        ...
        const infoSecurity = surveyResult.questions.find((el) => el._id === 'on-security-check');
        const guida = infoSecurity.responses.find((el) => el._id === 'on-security-drive');
        const norme = infoSecurity.responses.find((el) => el._id === 'on-security-security');
        const video = infoSecurity.responses.find((el) => el._id === 'on-security-video');

        const internalUserPrivacy = surveyResult.questions.find((el) => el._id === 'on-security-attachment');
        const onPrivacyPlace = internalUserPrivacy.responses.find((el) => el._id === 'on-security-place');
        const onPrivacyDate = internalUserPrivacy.responses.find((el) => el._id === 'on-security-date');
        ...
        const html = await fastify
          .view('pages/ricevuta-info-sicurezza', {
            surveyResult,
