  ...
  fastify.get(
    ...
    errorHandler(async (request, reply) => {
      ...
      try {
        ...
        const onFiscalVatCom = surveyResult.questions.find((el) => el._id === 'on-fiscal-vat-company');
        const onFiscalVatCompanyName = onFiscalVatCom.responses.find((el) => el._id === 'on-fiscal-vat-company-name');
        const oFiscalVatCompanyRep = onFiscalVatCom.responses.find((el) => el._id === 'on-fiscal-vat-company-rep');
        const onFiscalVatCompanyCg = onFiscalVatCom.responses.find((el) => el._id === 'on-fiscal-vat-company-cg');

        const internalUserPrivacy = surveyResult.questions.find((el) => el._id === 'on-require-attachment-company');
        const onPrivacyPlace = internalUserPrivacy.responses.find((el) => el._id === 'on-require-attach-cplace');
        const onPrivacyDate = internalUserPrivacy.responses.find((el) => el._id === 'on-require-attach-cdate');
        ...
        const html = await fastify
          .view('pages/autocert-per-giuridica', {
            surveyResult,
            customer: customerInfo,
            type: survey.type,
            sections: sectionsFinalList,
            moment,
            onFiscalVatCompanyName,
            oFiscalVatCompanyRep,
            onFiscalVatCompanyCg,
            luogoData,
          })
