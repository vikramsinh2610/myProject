  ...
  fastify.get(
    ...
    errorHandler(async (request, reply) => {
      ...
      try {
        ...
        const  onFiscalVatNumber = surveyResult.questions.find((el) => el._id === 'on-fiscal-vat-number');
        const onFiscalReg = onFiscalVatNumber.responses.find((el) => el._id === 'on-fiscal-reg');
        ...
        const internalUserPrivacy = surveyResult.questions.find((el) => el._id === 'on-privacy-attachment');
        const onPrivacyPlace = internalUserPrivacy.responses.find((el) => el._id === 'on-privacy-place');
        const onPrivacyDate = internalUserPrivacy.responses.find((el) => el._id === 'on-privacy-date');
        ...
        const onRuiCanceled = surveyResult.questions.find((el) => el._id === 'on-rui-canceled');
        const onRuiCanceledNo = onRuiCanceled.responses.find((el) => el._id === 'on-rui-canceled-no');
        const onRuiCanceledReason = surveyResult.questions.find((el) => el._id === 'on-rui-canceled-reason');
        const onRuiReason = onRuiCanceledReason.responses.find((el) => el._id === 'on-rui-reason');

        const html = await fastify
          .view('pages/scheda-anagrafica', {
            surveyResult,
