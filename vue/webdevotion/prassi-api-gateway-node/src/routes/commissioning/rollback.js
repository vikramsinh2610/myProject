...
module.exports = (fastify, opts, next) => {
  const options = {
    ...
    schema: {
      summary: 'Commissioning Rollback Confirmation',
      description: 'Rollback commissioning confirmation',
      tags: ['commissioning'],
      params: {
        type: 'object',
        properties: {
  ...
  fastify.post('/', options, errorHandler(async (request, reply) => {
    ...
    logRepository.insert(
      ...
      new LogEvent({
        ...
        description: `ROLLBACK commissioning ${request.params.commissioningId}`,
      }),
    );

    const processingState = await commissioningFlowService
      .startRollback(request.params.commissioningId)
      .then((state) => state);
