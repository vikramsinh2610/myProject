  ...
  fastify.post('/', options, errorHandler(async (request, reply) => {
    ...
    logRepository.insert(
      ...
      new LogEvent({
        ...
        description: `RESET commissioning ${request.params.commissioningId}`,
      }),
    );

    const processingState = await commissioningFlowService
      .reset(request.params.commissioningId)
      .then((state) => state);
