...
const filterByUser = (query, networkIds, productivePeriodMonth, productivePeriodYear) => {
  query
    .join('person_owner AS po', 'po.personId', 'consulting.customerId')
...
module.exports = (fastify, opts, next) => {
  ...
  fastify.get('/', options, async (request, reply) => {
    ...
    if (request.query.mode === 'summary') {
      const query = sql(CONSULTING)
        .countDistinct('consulting.id as pc')
        .countDistinct('consulting.promoterId as promoters')
        .countDistinct('consulting.customerId as customers');
    ...
    const query = sql.select('data').from(CONSULTING);

    filterByTime(query, period);
    filterByUser(query, networkIds, productivePeriodMonth, productivePeriodYear);

    const list = await query.orderBy('creationDate', 'desc').pluck('data');

    // load all related promoters
    const allPromoters = new Set();

    for (const item of list) {
      const { promoterId } = item.promoter;
