<template>
  <div class="fill-available column">
    <prassi-configuration-person-owner-list
      ...
      @deletePersonOwner="deletePersonOwnerClicked"
...
import PrassiConfigurationPersonOwnerList from '../components/configuration/prassi-configuration-person-owner-list';

export default {
  name: 'PersonsDetailHistory',
  components: {
    PrassiConfigurationPersonOwnerList,
  },
  data() {
    return {
  ...
  methods: {
    ...mapActions({
      ...
      fetchPersonOwners: 'configuration/fetchPersonOwners',
      savePersonOwner: 'configuration/savePersonOwner',
      deletePersonOwner: 'configuration/deletePersonOwner',
    ...
    comfirmAddCustomerInsurer() {
      if (this.form.networkId) {
        ...
        this.savePersonOwner(customer).then(() => {
          this.$q.notify({
    ...
    deletePersonOwnerClicked(customer) {
      this.$utils.logobj('CUSTOMER-DETAIL-HISTORY', 'deletePersonOwnerClicked', customer);
    ...
    deleteDossierInsurerConfirmed() {
      ...
      this.deletePersonOwner(this.customerToDelete).then(() => {
        this.$q.notify({
          message: this.$t('default.deleteOk'),
          color: 'secondary',
          timeout: 300,
    ...
    loadMoreCustomers({ index, done }) {
      if (this.last || this.error) {
        ...
        this.fetchPersonOwners(this.filter.fields).finally(() => done());
      }
    },
  },
};
