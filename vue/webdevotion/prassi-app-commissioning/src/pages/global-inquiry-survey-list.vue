...
export default {
  ...
  computed: {
    ...mapState({
      ...
      surveySummary: (state) => state.surveys.inquirySummary,
  ...
  methods: {
    ...mapActions({
      ...
      fetchInquirySurveySummary: 'surveys/fetchInquirySurveySummary',
    ...
    async computeList() {
      ...
      this.pendingListFetch = this.fetchInquirySurveyResults({ time: this.filter.time });
      await this.pendingListFetch;
      this.pendingList = false;

      this.surveys = this.surveyResults.map((item) => {
    ...
    async computeSummary() {
      ...
      await this.fetchInquirySurveySummary({ time: previousTime });
      this.pendingSummary = false;

      const currentSummary = {
        promoters: new Set(),
        customers: new Set(),
        pc: this.surveyResults.length,
      };

      this.surveyResults.forEach((item) => {
      ...
      this.currentSummary = [
        {
          ...
          previousValue: this.surveySummary.promoters,
          percentage: percentage(this.surveySummary.promoters, currentSummary.promoters.size),
        },
        {
          _id: 'customers',
          title: 'dossiers.customers',
          value: currentSummary.customers.size,
          previousValue: this.surveySummary.customers,
          percentage: percentage(this.surveySummary.customers, currentSummary.customers.size),
        },
        {
          _id: 'pc',
          title: 'dossiers.pc',
          value: currentSummary.pc,
          previousValue: this.surveySummary.pc,
          percentage: percentage(this.surveySummary.pc, currentSummary.pc),
        },
      ];
    },
  },
  ...
  data() {
    const redirectToSurvey = (e) => {
      this.$router.push(`/persons/${e.data.customerId}/survey-results/${e.data._id}`);
    };
    ...
    const deleteSurvey = async (id) => {
      this.pendingList = true;
      await this.deleteSurvey({ surveyId: id });
      this.pendingList = false;
      this.computeData();
    };
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          headerName: '',
          onCellClicked(e) {
            deleteSurvey(e.data.customerId);
          },
          cellRenderer() {
