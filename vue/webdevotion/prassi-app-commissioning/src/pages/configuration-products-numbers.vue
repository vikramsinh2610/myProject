...
export default {
  name: 'ConfigurationProductsNumbers',
  data() {
    return {};
  },
  computed: {
  ...
  methods: {
    uploadFile(files) {
      ...
      reader.addEventListener('load', async () => {
        ...
        await this.$utils.getApiCall(store, {
          url: `/v1/product-configurations/upload-numbers`,
          action: 'PUT',
          body: {
            file,
          },
        ...
        this.$q.notify({
          message: 'Tabella numeriche aggiornata',
          color: 'secondary',
          timeout: 2000,
        });
      });
    ...
    async downloadCsv() {
      ...
      const file = await this.$utils.getApiCall(store, {
        url: `/v1/product-configurations/download-numbers`,
      });

      await download(file, 'numbers.csv');
    },
  },
};
</script>
