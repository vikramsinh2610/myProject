...
export default {
  ...
  mounted() {
    this.$utils.log('PROPOSALS', 'mounted');
    ...
    this.fetchProposalsSummary(this.filter.fields.promoterId || this.loginId).finally(
      // eslint-disable-next-line no-return-assign
      () => (this.pendingSummary = false),
    );
    this.fetchProposalsSummaryPrevious(this.filter.fields.promoterId || this.loginId).finally(
      // eslint-disable-next-line no-return-assign
  ...
  methods: {
    ...mapActions({
      fetchProposalsSummary: 'dossiers/fetchProposalsSummary',
      fetchProposalsSummaryPrevious: 'dossiers/fetchProposalsSummaryPrevious',
      fetchProposals: 'dossiers/fetchProposals',
      ...
      fetchProposalsExcelReport: 'dossiers/fetchProposalsExcelReport',
    ...
    downloadExport() {
      ...
      this.fetchProposalsExcelReport(this.filter.fields.promoterId || this.loginId).then(() => {
    ...
    viewDossier(dossier) {
      this.$utils.logobj('PROPOSALS', 'viewDossier', dossier);
    ...
    loadMoreDossiers({ index, done }) {
      if (this.last || this.error) {
        ...
        this.fetchProposals(this.filter.fields.promoterId || this.loginId).finally(() => {
          this.pendingList = false;
          done();
        });
      }
