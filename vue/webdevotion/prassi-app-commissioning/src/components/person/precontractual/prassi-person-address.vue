  ...
  <q-card inline flat color="white" text-color="primary">
    <q-card-section>
      <div class="p-pc-title-section" v-if="!embedded">
        {{ $t('personPrecontractual.address') }}
      </div>
      <div>
        ...
        <div class="row justify-between q-my-xs">
          <q-input
            ...
            :readonly="readOnly"
            type="text"
            :label="$t('personPrecontractual.address')"
      ...
      <div class="row justify-between q-my-xs">
        <div class="map col-12" style="height: 500px; display: none"></div>
      </div>
    </q-card-section>
  ...
  methods: {
    ...
    saveData() {
      ...
      if (!this.$v.form.$error) {
        ...
        this.$emit('changePrecontractual', { stepper: { name: 'address', status: 'completed' } });
      } else {
        this.$emit('changePrecontractual', { stepper: { name: 'address', status: 'uncompleted' } });
