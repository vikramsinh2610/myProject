...
export default {
  name: 'PrassiPromoterLetterBonusPas',
  data() {
    return {
      pagination: {
        sortBy: undefined,
  ...
  watch: {
    ...
    bonusPas: {
      immediate: true,
      handler(bonusPas) {
        this.$utils.logobj('PRASSI-LETTER-COMMISSIONING-PA', 'bonusPas', bonusPas);

        // eslint-disable-next-line sonarjs/cognitive-complexity
        bonusPas.bonusTableRows.forEach((cb) => {
          const rowBonus = {
            from: cb.from / 100,
            to: cb.to / 100,
  ...
  methods: {
    ...
    submit() {
      ...
      if (!this.$v.form.$error) {
        ...
        const changedBonusPasBonusMonthly = [];
        this.tableData.forEach((row) => {
          ...
          changedBonusPasBonusMonthly.push(bonusMonthly);
        });
        ...
        this.$emit('changeData', {
          ...
          bonusPas: {
            bonusTableRows: changedBonusPasBonusMonthly,
          },
          productivePeriod,
        });
      } else {
