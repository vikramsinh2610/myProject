        ...
        <div class="row justify-between q-my-xs">
          <div class="col-5 row">
            <q-btn-toggle
              ...
              text-color="primary"
              :options="[
                { label: $t('dossiers.all'), value: 'all' },
                {
...
export default {
  name: 'PrassiPromoterLetterRappelPa',
  data() {
    return {
      pagination: {
        sortBy: undefined,
  ...
  watch: {
    ...
    rappelPa: {
      immediate: true,
      handler(rappelPa) {
        this.$utils.logobj('PRASSI-LETTER-COMMISSIONING-PA', 'rappelPa', rappelPa);

        this.form.rappelPaType = rappelPa.rappelPaType || 'all';
        // eslint-disable-next-line sonarjs/cognitive-complexity
        rappelPa.quarterTargetTableRows.forEach((cb) => {
    ...
    submit() {
      ...
      if (!this.$v.form.$error) {
        ...
        this.tableDataQuarter.forEach((row) => {
          ...
          changedRappelPaQuarter.push(bonusQuarter);
        });
        ...
        this.$emit('changeData', {
          ...
          rappelPa: {
            rappelPaType: this.form.rappelPaType,
            quarterTargetTableRows: changedRappelPaQuarter,
          },
          productivePeriod,
        });
      } else {
