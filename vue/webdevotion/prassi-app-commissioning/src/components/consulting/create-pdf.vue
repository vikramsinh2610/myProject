<template>
  <div>
    <section v-if="!signature">
      ...
      <div style="margin-top: 2.5em" v-if="isThirdPayer">
        <div class="p-pc-title-section">Terzo pagatore</div>

        <div class="row justify-between q-my-xs">
          <q-input
            class="col-5"
            v-model="thirdPayer.name"
            type="text"
            :label="$t('person.name')"
            :error-message="$t('person.error')"
            :error="$v.thirdPayer.name.$error"
            @blur="$v.thirdPayer.name.$touch"
            :readonly="hasSignature"
          />
          <q-input
            class="col-5"
            v-model="thirdPayer.surname"
            type="text"
            :label="$t('person.surname')"
            :error-message="$t('person.error')"
            :error="$v.thirdPayer.surname.$error"
            @blur="$v.thirdPayer.surname.$touch"
            :readonly="hasSignature"
          />
        </div>
        <div class="row justify-between q-my-xs">
          <q-input
            class="col-5"
            v-model="thirdPayer.email"
            type="email"
            :label="$t('person.email')"
            :error-message="$t('person.error')"
            :error="$v.thirdPayer.email.$error"
            @blur="$v.thirdPayer.email.$touch"
            :readonly="hasSignature"
          />
          <q-input
            class="col-5"
            v-model="thirdPayer.mobilePhone"
            type="text"
            :label="$t('person.mobilePhone')"
            :error-message="thirdPayer.errorMessage.mobilePhone"
            :error="$v.thirdPayer.mobilePhone.$error"
            @blur="$v.thirdPayer.mobilePhone.$touch"
            :readonly="hasSignature"
          />
        </div>
      </div>
    ...
    <section v-if="signature">
      <div v-if="signature.procedureCompleted">
...
export default {
  ...
  async mounted() {
    ...
    this.$set(this.customer, 'mobilePhone', this.person.mobilePhone.replace('+39', ''));

    if (this.consulting?.sepa.linkedCustomerId) {
      this.isThirdPayer = true;
      const thirdPayerId = this.consulting?.sepa.linkedCustomerId;
      await this.fetchPersonRelated(thirdPayerId);

      this.$set(this.thirdPayer, 'id', thirdPayerId);
      this.$set(this.thirdPayer, 'name', this.personRelated.name);
      this.$set(this.thirdPayer, 'surname', this.personRelated.surname);
      this.$set(this.thirdPayer, 'email', this.personRelated.email);
      this.$set(this.thirdPayer, 'mobilePhone', this.personRelated.mobilePhone.replace('+39', ''));
    }
    ...
    this.$set(this.promoter, 'mobilePhone', this.promoterData.mobilePhone.replace('+39', ''));
    ...
    if (this.signature && this.signature.customer) {
      ...
      this.$set(
        this.customer,
        'mobilePhone',
        this.signature.customer.mobilePhone.replace('+39', ''),
      );
    }
    ...
    if (this.signature && this.signature.promoter) {
      ...
      this.$set(this.promoter, 'mobilePhone', this.signature.promoter.mobilePhone).replace(
        '+39',
        '',
      );
    }
  ...
  computed: {
    ...mapState({
      ...
      personRelated: (state) => state.surveys.personRelated,
  ...
  methods: {
    ...
    async clickSign() {
      ...
      const customer = {
        ...
        surname: this.customer.surname.trim(),
        email: this.customer.email.trim(),
        mobilePhone: this.formatPhone(this.customer.mobilePhone),
        isCompany: this.person.isCompany,
      };

      const thirdPayer = this.isThirdPayer
        ? {
            id: this.thirdPayer.id,
            name: this.thirdPayer.name.trim(),
            surname: this.thirdPayer.surname.trim(),
            email: this.thirdPayer.email.trim(),
            mobilePhone: this.formatPhone(this.thirdPayer.mobilePhone),
            isCompany: this.thirdPayer.isCompany,
          }
        : { id: '', name: '', surname: '', mobilePhone: '', email: '' };

      const promoter = {
        id: this.promoterId,
        name: this.promoter.name.trim(),
        surname: this.promoter.surname.trim(),
        email: this.promoter.email.trim(),
