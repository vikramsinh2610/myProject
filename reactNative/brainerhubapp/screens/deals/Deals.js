...
import dealShoes from '../../assets/images/dealshoes.jpg';
...
export default function Deals({ navigation, route }) {

  const [status, setStatus] = useState('All');
  const [category, setCategory] = useState([]);
  const [filterCategory, setFilterCategory] = useState([]);
  const [isBestDeal, setBestDeal] = useState(false);
  ...
  const renderProductItem = ({ item }) => {
    ...
    return <TouchableOpacity style={styles.productBox} onPress={() => navigation?.navigate('ProductDetails', { productdetails: item })}>
      <View>
        <ImageBackground
          source={item.image}
          resizeMode="cover"
 ...
 */
  ...
  return (
    <View style={{ backgroundColor: '#fff' }}>
      <Header navigation={navigation} headerName="Deals" />
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.innerTemplate}>
          <Search navigation={navigation} placeHolder="Search" />
          {!isBestDeal ? <View style={styles.blockSlider}>
            <View style={styles.moduleHeading}>
              <Text style={styles.moduleTitle}>Categories on sale</Text>
            </View>
          ...
          {isBestDeal ?
            <>
              <View style={styles.moduleHeading}>
                <Text style={[styles.moduleTitle,{fontWeight:"bold",color:"black"}]}>Best Deals</Text>
              </View>
              <View style="productModule">
                <SafeAreaView>
                  <FlatList
...
const styles = StyleSheet.create({
  ...
  wishList:
    { marginBottom: 20, width: 20, height: 20, fontSize: 20 }
  ,
  productBoxOff: {
